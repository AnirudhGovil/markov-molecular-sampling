...
if __name__ == '__main__':
    ...
    parser.add_argument('--sampler', type=str,
                        default='sa', help='mcmc sampling algorithm')
    parser.add_argument('--proposal', type=str,
                        default='editor', help='how to pose proposals')
    parser.add_argument('--objectives', type=str,
                        default='gsk3b,jnk3,qed,sa')
    parser.add_argument('--score_wght', type=str,
                        default='  1.0, 1.0, 1.0, 1.0')
    parser.add_argument('--score_succ', type=str,
                        default='  0.5, 0.5, 0.6, .67')
    parser.add_argument('--score_clip', type=str,
                        default='  0.6, 0.6, 0.7, 0.7')
    ...
    log.getLogger().addHandler(log.FileHandler(
        os.path.join(config['run_dir'], 'log.txt'), mode='w'))
    log.info(str(config))
    ...
    for run in range(config['num_runs']):
        ...
        if config['editor_dir'] is not None:  # load pre-trained editor
            path = os.path.join(config['root_dir'],
                                config['editor_dir'], 'model_best.pt')
            editor.load_state_dict(torch.load(
